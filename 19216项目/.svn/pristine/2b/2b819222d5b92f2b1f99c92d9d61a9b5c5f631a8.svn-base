...
function initData(pageNumber){
	...
	$("#table-container").load(basePath + "/shgl/jmxx/index.do",{
		'ajaxCmd': 'table', 
...
function goAdd(msg){
	...
	top.layer.open({
	   ...
	   // shadeClose: true,
	    area: ['90%', '90%'],
	    content: basePath + '/shgl/jmxx/goAddEdit.do',
	    end:function(){
	    	initData(1);
		  }
	  });
...
function goUpdate(msg){
	...
	top.layer.open({
	   ...
	   // shadeClose: true,
	    area: ['90%', '90%'],
	    content: basePath+'/shgl/jmxx/goAddEdit.do?id='+code[0],
	    end:function(){
	    	initData(1);
		  }
	  });
...
function showImportView(msg){
	...
	top.layer.open({
	   ...
	   // shadeClose: true,
	    area: ['30%', '30%'],
	    content: basePath + '/shgl/jmxx/shouImportView.do',
	    end:function(){
	    	initData(1);
		  }
	  });
...
function goUpdateDq(id){
	...
	top.layer.open({
	   ...
	   // shadeClose: true,
	    area: ['90%', '90%'],
	    content: basePath+'/shgl/jmxx/goAddEdit.do?id='+id,
	    end:function(){
	    	initData(1);
		  }
	  });
...
function goDelDq(codes){
	top.layer.confirm("确定要删除所选记录吗？", {
		...
		yes: function(index){
			...
			$.ajax({
				type : "POST",
				url : basePath + "/shgl/jmxx/goDel.do",
				data : {'ids':codes},
...
function goExport(){
	...
	top.layer.confirm("确定要导出信息吗？", {
		...
		yes : function(index) {
			...
			window.location.href = basePath + "/shgl/jmxx/export.do?commId=" 
			+commId+'&flag=export'
			+'&ssId='+ssId+'&gridId='+gridId+'&xqId='+xqId+'&name='+name+'&inmateType='+category;
		},
		cancel : function(index) {
			layer.close(index);
		}
	...
	//window.location.href =  basePath+'/shgl/jmxx/export.do?flag=export';
}