...
var ue = UE.getEditor("content", {
	toolbars : [ [ "fullscreen", "source", "undo", "redo", "bold", "italic",
			...
			"splittocols", "searchreplace", "simpleupload", "insertvideo",
			"forecolor", "indent" ] ],
...
function doSave() {
	...
	var contentText = ue.getPlainTxt();// 获取带格式的纯文本
	$("#contentText").val(contentText);
	$.ajax({
		type : 'POST',
		url : basePath + '/dflz/dzyw/doSave.do?id=' + id,
...
function delFj(fjId, obj) {
	...
	top.layer.confirm("确定要删除所选择的附件吗？", {
		...
		yes : function(index) {
			$.ajax({
				type : 'POST',
				url : basePath + '/dflz/dzyw/delFj.do',
				data : {
					'fjId' : fjId,
					'id' : id
				},
