...
function doSave(){
	...
	$.ajax({
      	type: 'POST',
      	url: basePath + '/xtbg/tzgg/doSave.do?fName='+$("#fName").val()+'&id='+$("#id").val(),
...
//        $.ajax({
//        	type: 'POST',
//        	url: basePath + '/xtbg/tzgg/doSave.do?id='+$("#id").val(),
//        	data: $("#addForm").serialize(),
//        	success: function(data){
//        		top.layer.close(msgIndex);
//        		if(data=='success'){
...
function delFj(fjId, obj){
	top.layer.confirm("确定要删除所选择的附件吗？", {
		...
		yes: function(index){
			$.ajax({
				type: 'POST',
				url: basePath + '/xtbg/tzgg/delFj.do',
				data: {'fjId': fjId},
				success: function(data){
					if(data == 'success'){
						top.layer.msg('删除成功', {
