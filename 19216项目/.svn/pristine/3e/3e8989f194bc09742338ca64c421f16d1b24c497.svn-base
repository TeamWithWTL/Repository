...
function initData(pageNumber){
	...
	$("#table-container").load(basePath + "/shgl/sqgl/index.do",{'ajaxCmd': 'table', 'pageNumber': pageNumber, 'name': $("#name").val(), 'manager':$("#manager").val(), 'phone':$("#phone").val()}, function(){
...
function goAdd(msg){
	...
	top.layer.open({
	   ...
	   // shadeClose: true,
	    area: ['80%', '80%'],
	    content: basePath + '/shgl/sqgl/goAddEdit.do',
	    end:function(){
	    	initData(1);
		  }
	  });
...
function goUpdate(msg){
	...
	top.layer.open({
	   ...
	   // shadeClose: true,
	    area: ['80%', '80%'],
	    content: basePath+'/shgl/sqgl/goAddEdit.do?id='+code[0],
	    end:function(){
	    	initData(1);
		  }
	  });
...
function goUpdateDq(id){
	...
	top.layer.open({
	   ...
	   // shadeClose: true,
	    area: ['80%', '80%'],
	    content: basePath+'/shgl/sqgl/goAddEdit.do?id='+id,
	    end:function(){
	    	initData(1);
		  }
	  });
...
function goDelDq(codes){
	top.layer.confirm("确定要删除所选记录吗？", {
		...
		yes: function(index){
			...
			$.ajax({
				type : "POST",
				url : basePath + "/shgl/sqgl/goDel.do",
				data : {'ids':codes},
...
function goImpFile(){
	...
	top.layer.open({
	   ...
	   // shadeClose: true,
	    area: ['50%', '40%'],
	    content: basePath+'/shgl/sqgl/shouImportView.do',
	    end:function(){
	    	initData(1);
		  }
	  });
...
function goExport(){
	...
	top.layer.confirm("确定要导出信息吗？", {
		...
		yes : function(index) {
			...
			window.location.href = basePath + '/shgl/sqgl/export.do?flag=export'
			+ '&name='+name;
		},
		cancel : function(index) {
			layer.close(index);
