...
//		time : 0
//	});
	$("#table-container").load(basePath + "/app/dflz/appComp/toComp.do", {
		'ajaxCmd' : 'table',
...
//	});
	...
	$.ajax({
				async : true,
				type : 'POST',
				url : basePath + "/app/dflz/appComp/compNextContent.do",
				data : {
					'pageNumber' : pageNumber,
					'title' : title,
				},
 ...
 */
/*function detail(id){
	window.location.href = basePath +"/dflz/appComp/goView.do?id="+id;
}*/
...
mui("#table-container").on('tap','a',function(){
	 var zwxxId = this.getAttribute("name");
		 mui.openWindow({
			    url: basePath + "/app/dflz/appComp/goView.do?id="+zwxxId, 
			    id:'goView'
			  });
});
 ...
 */
/*function goAdd(){
	window.location.href = basePath +"/app/dflz/appComp/toAdd.do";
}*/
//添加事件按钮
mui("#addComp").on('tap','a',function(){
	  mui.openWindow({
		    url: basePath + '/app/dflz/appComp/toAdd.do', 
		    id:'addReport'
		  });
});