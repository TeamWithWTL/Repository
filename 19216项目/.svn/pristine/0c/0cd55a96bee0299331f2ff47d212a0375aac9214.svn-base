...
//		time : 0
//	});
	$("#table-container").load(basePath + "/app/xtbg/rdxw/index.do", {
		'ajaxCmd' : 'table',
...
/********************************-跳转--详情*******************************************/
   mui("#table-container").on('tap','a',function(){
	 var rdxwId = this.getAttribute("name");
	 mui.openWindow({
	    url: basePath + "/app/xtbg/rdxw/goView.do?id="+rdxwId, 
	    id:'goView'
	  });
	});
/**
...
//	});
	...
	$.ajax({
			async : true,
			type : 'POST',
			url : basePath + "/app/xtbg/rdxw/rdxwNextContent.do",
			data : {
				'pageNumber' : pageNumber,
				'title' : title,
			},
