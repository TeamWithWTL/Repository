...
//		time : 0
//	});
	$("#table-container").load(basePath + "/app/shzz/zxzm/toZxzm.do", {
		'ajaxCmd' : 'table',
...
/********************************-跳转--详情*******************************************/
	mui("#table-container").on('tap','a',function(){
	 var zxzmId = this.getAttribute("name");
	 mui.openWindow({
	    url: basePath + "/app/shzz/zxzm/goView.do?id="+zxzmId, 
	    id:'goView'
	  });
	});
/**
...
//	});
	...
	$.ajax({
			async : true,
			type : 'POST',
			url : basePath + "/app/shzz/zxzm/zxzmNextContent.do",
			data : {
				'pageNumber' : pageNumber,
				'title' : title,
			},
