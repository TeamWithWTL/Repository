...
function initData(pageNumber) {
	...
	$("#table-container").load(basePath + "/dflz/dzyw/index.do", {
		'ajaxCmd' : 'table',
...
function goAdd(msg) {
	...
	top.layer
			.open({
				...
				// shadeClose: true,
				area : [ '80%', '80%' ],
				content : basePath + '/dflz/dzyw/goAddEdit.do',
				end : function() {
					initData(1);
				}
			});
...
function goUpdate(msg) {
	...
	top.layer
			.open({
				...
				// shadeClose: true,
				area : [ '80%', '80%' ],
				content : basePath + '/dflz/dzyw/goAddEdit.do?id='
						+ id,
				end : function() {
					initData(1);
				}
...
function goDel(){
	...
	top.layer.confirm("确定要删除选择的记录吗？", {
		...
		yes: function(index){
			...
			$.ajax({
				type : "POST",
				url : basePath + "/dflz/dzyw/doDel.do",
				data : {'id':ids},
...
function goAudit(id, status) {
	...
	top.layer
			.open({
				...
				// shadeClose: true,
				area : [ '80%', '80%' ],
				content : basePath + '/dflz/dzyw/goViewAudit.do?id=' + id,
				end : function() {
					initData(1);
				}
			});
...
function update(id) {
	...
	top.layer
			.open({
				...
				// shadeClose: true,
				area : [ '80%', '80%' ],
				content : basePath + '/dflz/dzyw/goAddEdit.do?id=' + id,
				end : function() {
					initData(1);
				}
			});
