...
function goAdd(msg){
	...
	top.layer.open({
	    ...
	    fix: false,
	    area: ['80%', '80%'],
	    content: basePath + '/shzz/zxjs/goAddEdit.do',
	    end:function(){
	    	initData(1);
		  }
	  });
...
function goUpdate(msg){
	...
	top.layer.open({
	   ...
	   // shadeClose: true,
	    area: ['80%', '80%'],
	    content: basePath+'/shzz/zxjs/goAddEdit.do?id='+id[0].split(",")[0],
	    end:function(){
	    	initData(1);
		  }
	  });
...
function goDel(){
	...
	top.layer.confirm("确定要删除选择的记录吗？", {
		...
		yes: function(index){
			...
			$.ajax({
				type : "POST",
				url : basePath + "/shzz/zxjs/doDel.do",
				data : {'ids':ids},
...
function goAuditing(){
	...
	top.layer.confirm("确定要审核所选记录吗？", {
		...
		yes: function(index){
			...
			$.ajax({
				type : "POST",
				url : basePath + "/shzz/zxjs/doAuditing.do",
				data : {'ids':ids},
				error : function(request) {
					top.layer.close(msgIndex);
...
function antiaudit(id) {
	$.ajax({
		type : "POST",
		url : basePath + "/shzz/zxjs/antiaudit.do",
		data : {
			'id' : id
		},
		error : function(request) {
...
function goSh(id){
	...
	top.layer.open({
	   ...
	   // shadeClose: true,
	    area: ['80%', '80%'],
	    content: basePath+'/shzz/zxjs/goSh.do?id='+id,
	    end:function(){
	    	initData(1);
		  }
	  });
