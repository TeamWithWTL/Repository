...
function initData(){
	...
	$("#table-container").load(basePath + "/app/shgl/wggl/index.do",{
		'ajaxCmd': 'table', 
		'pageNumber': pageNumber, 
		'name': $("#name").val(),
		'commId' : $("#wgCode1 option:selected").val(),
		'serId' : $("#ssId option:selected").val()
		}, function(){
...
//跳转--详情
mui("#table-container").on('tap','#wgId',function(){
	 var wgId = this.getAttribute("name");
		 mui.openWindow({
			    url: basePath + "/app/shgl/wggl/goView.do?id="+wgId, 
			    id:'goView'
			  });
});
...
//	});
	...
	var commId = $("#wgCode1 option:selected").val();

	$.ajax({
				async : true,
				type : 'POST',
				url : basePath + "/app/shgl/wggl/wgglNextContent.do",
				data : {
					'pageNumber' : pageNumber,
					'name' : name,
					'commId' : commId,
...
//					top.layer.close(msgIndex);
					...
					for(var i = 0; i<listObj.length; i++){
						value = listObj[i];
						var html = '<div class="mui-card" id="wgId" name="'+value.wgId+'">'
