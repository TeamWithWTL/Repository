...
function initData(pageNumber){
	...
	$("#table-container").load(basePath + "/shgl/sqmy/index.do",{'ajaxCmd': 'table', 'pageNumber': pageNumber, 'title': $("#title").val()}, function(){
...
function goAdd(msg){
	...
	top.layer.open({
	   ...
	   // shadeClose: true,
	    area: ['65%', '80%'],
	    content: basePath + '/shgl/sqmy/goAddEdit.do',
	    end:function(){
	    	initData(1);
		  }
	  });
...
//跳转修改页面
function goUpdate(id,status){
	if(status == '2'){
		top.layer.confirm("已下发的数据不能修改",{
			 icon:0,
			 title:"提示"
		});
		return false;
	...
	top.layer.open({
	   ...
	   // shadeClose: true,
	    area: ['80%', '80%'],
	    content: basePath+'/shgl/sqmy/goAddEdit.do?id='+id,
	    end:function(){
	    	initData(1);
		  }
	  });
...
function goDel(){
	...
	top.layer.confirm("确定要删除选择的记录吗？", {
		...
		yes: function(index){
			...
			$.ajax({
				type : "POST",
				url : basePath + "/shgl/sqmy/doDel.do",
				data : {'ids':ids},
...
function issued(id){
	$.ajax({
		type : "POST",
		url : basePath + "/shgl/sqmy/issued.do",
		data : {'id':id},
		error : function(request) {
			top.layer.msg('数据下发失败，请联系管理员', {shade : [ 0.5, '#999999' ]});
		},
		success : function(data) {
			if (data == "success2") {
