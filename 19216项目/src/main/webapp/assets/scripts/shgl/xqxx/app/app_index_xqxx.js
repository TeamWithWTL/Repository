...
function initData(){
	...
	$("#table-container").load(basePath + "/app/shgl/xqxx/index.do",{
		'ajaxCmd': 'table', 
		'pageNumber': pageNumber, 
		'name': $("#name").val(),
		'commId' : $("#xqCode1 option:selected").val(),
...
//跳转--详情
mui("#table-container").on('tap','#xqxxId',function(){
	 var xqxxId = this.getAttribute("name");
		 mui.openWindow({
			    url: basePath + "/app/shgl/xqxx/goView.do?id="+xqxxId, 
			    id:'goView'
			  });
});
...
//	});
	...
	var name = $("#name").val();
	var commId = $("#xqCode1 option:selected").val();
	var ssId = $("#ssId option:selected").val();
	$.ajax({
				async : true,
				type : 'POST',
				url : basePath + "/app/shgl/xqxx/xqxxNextContent.do",
				data : {
					'pageNumber' : pageNumber,
					'name' : name,
					'commId' : commId,
...
//					top.layer.close(msgIndex);
					...
					for(var i = 0; i<listObj.length; i++){
						value = listObj[i];
						var html = '<div class="mui-card" id="xqxxId" name="'+value.xqxxId+'">'
