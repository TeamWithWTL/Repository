...
function initData(pageNumber){
	...
	$("#table-container").load(basePath + "/xtgl/sjzdgl/index.do",{'ajaxCmd': 'table', 'pageNumber': pageNumber, 'name': $("#name").val(), 'code': $("#code").val(), 'cdCode':$("#cdCode").val()}, function(){
...
function goAdd(msg){
	...
	top.layer.open({
	   ...
	   // shadeClose: true,
	    area: ['850px', '450px'],
	    content: basePath + '/xtgl/sjzdgl/goAddEdit.do',
	    end:function(){
	    	initData(1);
		  }
	  });
...
function goDel(){
	...
	top.layer.confirm("确定要删除所选数据吗？", {
		...
		yes: function(index){
			...
			$.ajax({
				type : "POST",
				url : basePath + "/xtgl/sjzdgl/goDel.do",
				data : {'ids':codes},
...
function goDescIndex(msg){
	...
	top.layer.open({
	   ...
	   // shadeClose: true,
	    area: ['90%', '90%'],
	    content: basePath+'/xtgl/sjzdgl/descIndex.do?code='+code[0]
	  });
}
...
function goSjzd(id){
	...
	top.layer.open({
	    type: 2,
	    title: "<div class='layerTitleDiv'></div><span class='layerTitleSpan'>添加明细</span>",
	    fix: false,
	   // shadeClose: true,
	    area: ['90%', '90%'],
	    content: basePath+'/xtgl/sjzdgl/descIndex.do?code='+id
	  });
}