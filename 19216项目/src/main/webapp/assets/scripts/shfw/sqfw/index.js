...
function initData(pageNumber){
	...
	$("#table-container").load(basePath + "/shfw/sqfw/index.do",{'ajaxCmd': 'table', 'pageNumber': pageNumber, 'title': $("#title").val()}, function(){
...
function goAuditing(){
	...
	top.layer.confirm("确定要审核所选记录吗？", {
		...
		yes: function(index){
			...
			$.ajax({
				type : "POST",
				url : basePath + "/shfw/sqfw/doAuditing.do",
				data : {'ids':ids},
...
function goView(id){
	...
	top.layer.open({
	   ...
	   // shadeClose: true,
	    area: ['80%', '80%'],
	    content: basePath+'/shfw/sqfw/goView.do?id='+id,
	  });
}
...
function goSh(id){
	...
	top.layer.open({
	   ...
	   // shadeClose: true,
	    area: ['80%', '80%'],
	    content: basePath+'/shfw/sqfw/goSh.do?id='+id,
	    end:function(){
	    	initData(1);
		}
	  });
...
function goDel(){
	...
	top.layer.confirm("确定要删除选择的记录吗？", {
		...
		yes: function(index){
			...
			$.ajax({
				type : "POST",
				url : basePath + "/shfw/sqfw/doDel.do",
				data : {'ids':ids},
				error : function(request) {
					top.layer.close(msgIndex);
...
function auditing(id) {
	$.ajax({
		type : "POST",
		url : basePath + "/shfw/sqfw/auditing.do",
		data : {
			'id' : id
		},
		error : function(request) {
...
function antiaudit(id) {
	$.ajax({
		type : "POST",
		url : basePath + "/shfw/sqfw/antiaudit.do",
		data : {
			'id' : id
		},
		error : function(request) {
...
function goModify(id){
	...
	top.layer.open({
	   ...
	   // shadeClose: true,
	    area: ['80%', '80%'],
	    content: basePath+'/shfw/sqfw/goAddEdit.do?id='+id,
	    end:function(){
	    	initData(1);
		}
	  });
