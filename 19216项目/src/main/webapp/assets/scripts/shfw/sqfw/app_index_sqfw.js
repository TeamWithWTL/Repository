...
//		time : 0
//	});
	$("#table-container").load(basePath + "/app/shfw/sqfw/toSqfw.do", {
		'ajaxCmd' : 'table',
...
/********************************-跳转--详情*******************************************/
mui("#table-container").on('tap','#sqfwXqId',function(){
	 var sqfwId = this.getAttribute("name");
		 mui.openWindow({
			    url: basePath + "/app/shfw/sqfw/goView.do?sqfwId="+sqfwId, 
			    id:'goView'
			  });
});
/**
...
//	});
	...
	$.ajax({
				async : true,
				type : 'POST',
				url : basePath + "/app/shfw/sqfw/sqfwNextContent.do",
				data : {
					'pageNumber' : pageNumber,
					'title' : title,
				},
