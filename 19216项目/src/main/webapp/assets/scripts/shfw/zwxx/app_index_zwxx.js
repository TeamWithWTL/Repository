...
//		time : 0
//	});
	$("#table-container").load(basePath + "/app/shfw/zwxx/toZwxx.do", {
		'ajaxCmd' : 'table',
...
/********************************-跳转--详情*******************************************/
mui("#table-container").on('tap','#zwxxXqId',function(){
	 var zwxxId = this.getAttribute("name");
		 mui.openWindow({
			    url: basePath + "/app/shfw/zwxx/goView.do?zwxxId="+zwxxId, 
			    id:'goView'
			  });
});
/**
...
//	});
	...
	$.ajax({
				async : true,
				type : 'POST',
				url : basePath + "/app/shfw/zwxx/zwxxNextContent.do",
				data : {
					'pageNumber' : pageNumber,
					'title' : title,
				},
