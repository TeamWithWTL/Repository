...
function initTzggApp() {
	...
	$("#table-container").load(basePath + "/app/xtbg/tzgg/index.do", {
		'ajaxCmd' : 'table',
...
/********************************-跳转--详情*******************************************/
   mui("#table-container").on('tap','a',function(){
	 var tzggId = this.getAttribute("name");
	 mui.openWindow({
	    url: basePath + "/app/xtbg/tzgg/goView.do?id="+tzggId, 
	    id:'goView'
	  });
	});
/**
...
function getContentNext() {
	...
	$.ajax({
			async : true,
			type : 'POST',
			url : basePath + "/app/xtbg/tzgg/tzggNextContent.do",
			data : {
				'pageNumber' : pageNumber,
				'title' : title,
			},
